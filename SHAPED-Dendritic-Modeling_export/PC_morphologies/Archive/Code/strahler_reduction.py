...
#        plot_sholl(branch_list)
        ...
        sholl_radii = np.ndarray.tolist(np.linspace(0,500, 11))
        j_list = []
#        Rth_list = []
        for compartment in compartment_list:
            for j in range(0, len(sholl_radii)):
                if compartment.start_coordinates[1] < sholl_radii[j] and compartment.end_coordinates[1] > sholl_radii[j]:
                    j_list.append(j)
                    top_half = 0
...
#                        Rth_list.append(top_half)
#                    else:
#                        Rth_list.append(compartment.branch.parent_bifurcation.Rth + top_half)

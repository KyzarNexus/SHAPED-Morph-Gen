...
#             last_compartment = (compartment.x, compartment.y, compartment.z)
    ...
    def set_branch_order(self):
        if self.daughter_bifurcation != []:
            self.daughter_bifurcation.order = self.branch_order
...
def reduce_backprop(branch):
    if branch.parent_bifurcation != []:
        stem_Rth = branch.pathlength
    else:
        stem_Rth = branch.pathlength + branch.parent_Rth
        
    if branch.daughter_bifurcation == []:
        return
    branch.daughter_bifurcation.parent_Rth = stem_Rth
    branch1_Rth = branch.daughter_bifurcation.daughter_branch[0].Rth
    branch2_Rth = branch.daughter_bifurcation.daughter_branch[1].Rth
    
    combined1 = stem_Rth*branch1_Rth/(stem_Rth + branch1_Rth)
    combined2 = stem_Rth*branch2_Rth/(stem_Rth + branch2_Rth)
    
    
    branch.daughter_bifurcation.daughter_branch[0].parent_Rth = combined2
    branch.daughter_bifurcation.daughter_branch[1].parent_Rth = combined1
    
    reduce_backprop(branch.daughter_bifurcation.daughter_branch[0])
    reduce_backprop(branch.daughter_bifurcation.daughter_branch[1])
    return

def reduce_parents(branch):
    if branch.parent_bifurcation != []:
        daughter1_Rth = branch.parent_bifurcation.daughter_branch[0].Rth
        daughter2_Rth = branch.parent_bifurcation.daughter_branch[1].Rth
        
        if daughter1_Rth == -1 or daughter2_Rth == -1:
            return
        
        branch.parent_bifurcation.d1_Rth = daughter1_Rth
        branch.parent_bifurcation.d2_Rth = daughter2_Rth
        
#        branch.parent_bifurcation.Rth = daughter1_Rth*daughter2_Rth/(daughter1_Rth + daughter2_Rth)
                
        branch.parent_bifurcation.parent_branch.Rth = daughter1_Rth*daughter2_Rth/(daughter1_Rth + daughter2_Rth) + branch.parent_bifurcation.parent_branch.pathlength
        reduce_parents(branch.parent_bifurcation.parent_branch)
    else:
        reduce_backprop(branch)

def strahler_reduction(compartment_list, branch_list, bifurcation_list, termination_list, s):
    s_compartments = []
    s_branches = []
    s_bifurcations = []
    s_terminations = []
    ...
    for termination in termination_list:
        termination.branch.Rth = termination.branch.pathlength
    for termination in termination_list:
        reduce_parents(termination.branch)
    
    return s_compartments, s_branches, s_bifurcations, s_terminations

file_list_of_lists = []
file_list1 = glob.glob('../Data/Morphologies/claiborne/OutlierRemoved/*.swc')
file_list2 = glob.glob('../Data/Morphologies/generator/*recreated*.swc')
file_list3 = glob.glob('../Data/Morphologies/l_neuron/*recreated*.swc')
file_list4 = glob.glob('../Data/Morphologies/flat/*recreated*.swc')
file_list_of_lists.append(file_list1)
file_list_of_lists.append(file_list2)
file_list_of_lists.append(file_list3)
file_list_of_lists.append(file_list4)
...
for iii in range(0, 4):
    file_list = file_list_of_lists[iii]
    ...
    #        plot_sholl(branch_list)
            ...
            if max_y == 0:
                if compartment.y < max_y:
                    max_y = compartment.y
            num_bins = 11
            s_compartments, s_branches, s_bifurcations, s_terminations = strahler_reduction(compartment_list, branch_list, bifurcation_list, termination_list, s)
            sholl_radii = np.ndarray.tolist(np.linspace(0,1, num_bins))
    #        j_list = []
    #        Rth_list = []
            for compartment in compartment_list:
#                for j in range(0, len(sholl_radii)):
#                    if compartment.start_coordinates[1] < max_y*sholl_radii[j] and compartment.end_coordinates[1] > max_y*sholl_radii[j]:
                if 1 == 1:
                    if random.random() < 0.015 and compartment.branch != []:
                        if compartment.end_coordinates[1] < 0:
                            continue
#    
    
                        top_half = 0
                        next_compartment = compartment
                        if next_compartment.daughter_compartment != []:     
                            while next_compartment.branch == next_compartment.daughter_compartment[0].branch:
                                top_half = top_half + next_compartment.length
                                next_compartment = next_compartment.daughter_compartment[0]
                                if next_compartment.daughter_compartment == []:
                                    break
                        bot_half = compartment.branch.pathlength - top_half
                                            
                        if compartment.branch.parent_bifurcation == []:
                            bot_Rth = (bot_half)
                        else:
                            bot_Rth = (compartment.branch.parent_bifurcation.Rth + bot_half)
                            
                        if compartment.branch.daughter_bifurcation == []:
                            top_Rth = top_half
                        else:
                            top_Rth = compartment.branch.daughter_bifurcation.Rth + top_half
                        if bot_Rth < 0 or top_Rth < 0:
                            continue
    
    #                    parallel_Rth = bot_Rth*top_Rth/(bot_Rth + top_Rth)
                        parallel_Rth = top_Rth/(bot_Rth + top_Rth)
                        if parallel_Rth > 1:
                            print top_Rth
                        j_list.append(compartment.end_coordinates[1]/max_y)
#                        j_list.append(j/(num_bins-1))
                        jorder_list.append(compartment.branch.branch_order)
                        Rth_list.append(parallel_Rth)
                            
                            
                
    #        print Rth_list
            for bifurcation in bifurcation_list:
                bifurcation.combined_Rth = 1/(1/bifurcation.d1_Rth + 1/bifurcation.d2_Rth + 1/bifurcation.parent_Rth)
            
    
                daughters_Rth = bifurcation.d1_Rth*bifurcation.d2_Rth/(bifurcation.d1_Rth + bifurcation.d2_Rth)
                
    #            Rth_list.append(bifurcation.combined_Rth)            
    #            Rth_list.append(bifurcation.parent_Rth/(daughters_Rth + bifurcation.parent_Rth))
    #            Rth_list.append(np.exp(-bifurcation.parent_branch.pathlength_to_soma/500))
    #            Rth_list.append(bifurcation.parent_Rth/(bifurcation.d1_Rth + bifurcation.d2_Rth + bifurcation.parent_Rth))
                
                
                bifurcation.bifurcating_compartment.y = bifurcation.bifurcating_compartment.y/max_y
                if bifurcation.bifurcating_compartment.y < 0:
                    bifurcation.bifurcating_compartment.y = -bifurcation.bifurcating_compartment.y            
                
                y_list.append(bifurcation.bifurcating_compartment.y)
                
                
                order_list.append(bifurcation.order)
    ...
    max_ysholl_list =  num_bins
    ysholl_avgs = [0]*max_ysholl_list
    ysholl_lnspc = [0]*max_ysholl_list
    ysholl_std = [0]*max_ysholl_list
    for ysholl_val in range(0, max_ysholl_list):
        ysholl_lnspc[ysholl_val] = ysholl_val/(num_bins-1)
        indices = [i for i, x in enumerate(j_list) if x == ysholl_val/num_bins]
        tmp_list = []
        for idx in indices:
            ysholl_avgs[ysholl_val] = ysholl_avgs[ysholl_val] + Rth_list[idx]/len(indices)
            tmp_list.append(Rth_list[idx])
        ysholl_std[ysholl_val] = np.std(tmp_list)
    
    ysholl_fit_coef = np.polyfit(j_list, Rth_list, 2)
    ysholl_fit = [ysholl_fit_coef[0]*(y_sholl_x**2)+ysholl_fit_coef[1]*y_sholl_x + ysholl_fit_coef[2] for y_sholl_x in ysholl_lnspc]
    #print ysholl_fit
    fit1 = ysholl_fit
    if iii == 0:
        fit1 = ysholl_fit
        ...
        fit3 = ysholl_fit
    fits.append(ysholl_fit)
    
#    plt.figure()
#    plt.plot(ysholl_lnspc, fit1, label='claiborne')
#    plt.plot(ysholl_lnspc, fit2, label='generated')
#    plt.plot(ysholl_lnspc, fit3, label='l_neuron')
#    plt.legend()
...
#    plt.show()
    ...
    plt.figure()
    plt.scatter(j_list, Rth_list, color='black', marker='.')
#    plt.scatter(ysholl_lnspc, ysholl_avgs, color='red', marker='o', s=80)
#    plt.errorbar(ysholl_lnspc, ysholl_avgs, ysholl_std, color='red', marker = 'o', elinewidth=3)
    plt.plot(ysholl_lnspc, ysholl_fit, linewidth=5)
    
    plt.xlabel('Normalized Y-Coordinate')
    plt.ylabel('Proximal Current Fraction')
    sns.despine()
    sns.set_context("talk")
    plt.savefig('../Figures/pcf{}.svg'.format(iii), format='svg', dpi=1200)

    plt.show()
    
    max_order_list = max(jorder_list) + 1
    order_avgs = [0]*max_order_list
    order_lnspc = [0]*max_order_list
    for order_val in range(0, max_order_list):
        order_lnspc[order_val] = order_val
        indices = [i for i, x in enumerate(jorder_list) if x == order_val]
        for idx in indices:
            order_avgs[order_val] = order_avgs[order_val] + Rth_list[idx]/len(indices)
    
    
    branch_fit_coef = np.polyfit(jorder_list, Rth_list, 2)
    branch_fit = [branch_fit_coef[0]*(branch_order_x**2)+branch_fit_coef[1]*branch_order_x + branch_fit_coef[2] for branch_order_x in order_lnspc]
    
    bfits.append(branch_fit)
    
    #plt.figure()
    #order_lnspc = [0]*len(bfit1)
    #for order_val in range(0, len(bfit1)):
    #    order_lnspc[order_val] = order_val
    #plt.plot(order_lnspc, bfit1, label='claiborne')
    #order_lnspc = [0]*len(bfit2)
    #for order_val in range(0, len(bfit2)):
    #    order_lnspc[order_val] = order_val
    #plt.plot(order_lnspc, bfit2, label='generated')
    #order_lnspc = [0]*len(bfit3)
    #for order_val in range(0, len(bfit3)):
    #    order_lnspc[order_val] = order_val
    #plt.plot(order_lnspc, bfit3, label='l_neuron')
    #plt.legend()
    ...
    #plt.figure()
    #plt.scatter(jorder_list, Rth_list)
    #plt.scatter(order_lnspc, order_avgs)
    #plt.show()
    
    #plt.figure()
    #plt.scatter(y_list, Rth_list)
    #plt.xlabel('Normalized Y Coordinate')
    #plt.ylabel('Equivalent Resistance')
    #plt.show()
    #
    #max_order_list = max(order_list) + 1
    #order_avgs = [0]*max_order_list
    #order_lnspc = [0]*max_order_list
    #for order_val in range(0, max_order_list):
    #    order_lnspc[order_val] = order_val
    #    indices = [i for i, x in enumerate(order_list) if x == order_val]
    #    for idx in indices:
    #        order_avgs[order_val] = order_avgs[order_val] + Rth_list[idx]/len(indices)
    #
    #plt.figure()
    #plt.scatter(order_list, Rth_list)
    #plt.scatter(order_lnspc, order_avgs)
    #plt.xlabel('Branch Order')
    #plt.ylabel('Equivalent Resistance')
    #plt.show()
...
sns.set_style("white")
sns.set_style("ticks")
#sns.set_context("talk", font_scale = 1, rc={"lines.linewidth":5.5})
sns.set_context("talk")
...
#        color=next(palette)
    ...
    plt.plot(ysholl_lnspc, fits[i], color=next(palette))

    plt.xlabel(u"Normalized Y-Coordinate")
    plt.ylabel(u"Proximal Current Fraction")
sns.despine()
...
plt.xlim([0,1])
plt.ylim([0,1])
plt.savefig('../Figures/pcf_compare4.svg', format='svg', dpi=1200)
plt.show()
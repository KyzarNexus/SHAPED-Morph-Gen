...
observations_file = '..\\..\\Data\\Metrics\\{}_point_process.txt'.format(dataset_identifier)
observations_file_apical = '..\\..\\Data\\Metrics\\{}_point_process_apical.txt'.format(dataset_identifier)
observations_file_basal = '..\\..\\Data\\Metrics\\{}_point_process_basal.txt'.format(dataset_identifier)
# file_list = glob.glob('..\\..\\Data\\Morphologies\\claiborne\\*.swc')
# observations_file = '..\\..\\Data\\Metrics\\claiborne_point_process.txt'
# observations_file_apical = '..\\..\\Data\\Metrics\\empty_observations.txt'
# observations_file_basal = '..\\..\\Data\\Metrics\\claiborne_point_process.txt'
...
all_surface_area_apical = []

pair_table_apical = {}
id_pairs_apical = []
ordered_pairs_apical = []
...
all_surface_area_basal = []

pair_table_basal = {}
id_pairs_basal = []
ordered_pairs_basal = []

# avg_data = [] # Due to original comment
...
for i in range (0, len(file_list)):
    if i < 100:
        ...
        # Apical
        for bif in bifurcation_list_apical:
            if bif.parent_branch != []:
                if bif.parent_branch.parent_bifurcation != []:
                    pair_table_apical[bif.parent_branch.parent_bifurcation] = bif
    
        for bif in pair_table_apical:
            id_pairs_apical.append((bif.bifurcating_compartment.compartment_id, pair_table_apical[bif].bifurcating_compartment.compartment_id))
            ordered_pairs_apical.append((bif, pair_table_apical[bif]))
        ...
        # Basal
        for bif in bifurcation_list_basal:
            if bif.parent_branch != []:
                if bif.parent_branch.parent_bifurcation != []:
                    pair_table_basal[bif.parent_branch.parent_bifurcation] = bif
    
        for bif in pair_table_basal:
            id_pairs_basal.append((bif.bifurcating_compartment.compartment_id, pair_table_basal[bif].bifurcating_compartment.compartment_id))
            ordered_pairs_basal.append((bif, pair_table_basal[bif]))
        ...
        # plot_branches(branch_list_apical)
        # plot_branches(branch_list_basal)
# General
point_process_length_max = 0
point_process_list = []
## Apical
point_process_length_max_apical = 0
point_process_list_apical = []
# Basal
point_process_length_max_basal = 0
point_process_list_basal = []

# General
for termination in all_terminations_list:
    point_process_list.append(termination.branch.point_process)
    point_process_length = len(termination.branch.point_process)
    if point_process_length > point_process_length_max:
        point_process_length_max = point_process_length
        
    # pathlength_list = []
        # for morphology in all_morphologies_list:
        # for termination in morphology.terminal_list:
        #     pathlength_list.append(termination.euclidean_from_soma)
        # median_pathlength = np.median(pathlength_list)
        # point_process_entry = []
        # for termination in morphology.terminal_list:
        #     if termination.euclidean_from_soma == median_pathlength and point_process_entry == []:
        #         point_process_list.append(termination.branch.point_process)
        #         point_process_length = len(termination.branch.point_process)
        #         if point_process_length > point_process_length_max:
        #                         point_process_length_max = point_process_length
        ...
        # plt.figure()
# Apical
for termination in all_terminations_list_apical:
    point_process_list_apical.append(termination.branch.point_process)
    point_process_length_apical = len(termination.branch.point_process)
    if point_process_length_apical > point_process_length_max_apical:
        point_process_length_max_apical = point_process_length_apical
    ...
    # pathlength_list = []
        ...
        # plt.figure()
# Basal
for termination in all_terminations_list_basal:
    point_process_list_basal.append(termination.branch.point_process)
    point_process_length_basal = len(termination.branch.point_process)
    if point_process_length_basal > point_process_length_max_basal:
        point_process_length_max_basal = point_process_length_basal
...
# General
for i in range(0, len(point_process_list)):
    if len(point_process_list[i]) != point_process_length_max:
        point_process_list[i] = point_process_list[i] + [0]*(point_process_length_max - len(point_process_list[i]))

    #     plt.plot(point_process_list[i])
        # plt.show()
                
        # plt.figure()
        # n_subplot = len(point_process_list)
        # to_be_rastered = []
        # for i in range(0,n_subplot):
        # event_index_list = [x for x, y in enumerate(point_process_list[i]) if y == 1]
        # to_be_rastered.append(event_index_list)
        # ax = raster(to_be_rastered)
        # plt.xlabel('Distance of bifurcation occurrence')
        # plt.ylabel('trial')
# Apical
for i in range(0, len(point_process_list_apical)):
    if len(point_process_list_apical[i]) != point_process_length_max_apical:
        point_process_list_apical[i] = point_process_list_apical[i] + [0]*(point_process_length_max_apical - len(point_process_list_apical[i]))
    ...
    #     plt.plot(point_process_list[i])
        ...
        # plt.ylabel('trial')
# Basal
for i in range(0, len(point_process_list_basal)):
    if len(point_process_list_basal[i]) != point_process_length_max_basal:
        point_process_list_basal[i] = point_process_list_basal[i] + [0]*(point_process_length_max_basal - len(point_process_list_basal[i]))
...
#     for i in range(0, len(point_process_list)):
#         entries = ' '.join(str(x) for x in point_process_list[i])
#         strs = entries
#         f.write(strs + '\n')
#     f.closed
# Storing Apical
with open(observations_file_apical, 'w') as f:
    for i in range(0, len(point_process_list_apical)):
        entries = ' '.join(str(x) for x in point_process_list_apical[i])
        strs = entries
        f.write(strs + '\n')
    f.closed
# Storing Basal
with open(observations_file_basal, 'w') as f:
    for i in range(0, len(point_process_list_basal)):
        entries = ' '.join(str(x) for x in point_process_list_basal[i])
        strs = entries
        f.write(strs + '\n')
    f.closed
